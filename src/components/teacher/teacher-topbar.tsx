...
export function TeacherTopbar({ title, description }: TeacherTopbarProps) {
  return (
    <header className="flex items-center justify-between gap-4 shrink-0 mb-6">
      {/* Left: greeting */}
      <div>
        <h1 className="font-headline text-[28px] font-extrabold tracking-tight text-[#002f76] flex items-center gap-2">
          {title || "Welcome back, Sarah!"} <span className="text-[26px]">👋</span>
        </h1>
      ...
      <div className="flex items-center gap-3 shrink-0">
        ...
        <button className="relative flex h-[40px] w-[40px] shrink-0 items-center justify-center rounded-full bg-white shadow-sm border border-[#e2e8f0]/80 text-[#0050d5]">
          ...
          <span className="absolute right-[9px] top-[9px] h-2.5 w-2.5 rounded-full border-2 border-white bg-[#005cc8]" />
        </button>
