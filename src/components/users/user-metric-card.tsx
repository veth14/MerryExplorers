type MetricType = "total" | "active" | "leave" | "new";
...
const configs: Record<MetricType, {
  labelColor: string;
  metaIcon: string;
  metaColor: string;
  borderColor: string;
    ...
    graphic: (
      <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 80 60" className="w-full h-full">
        ...
        <circle cx="40" cy="28" r="22" fill="none" stroke="#2da05b" strokeWidth="5"
          strokeDasharray="96 138.2" strokeDashoffset="34.5" strokeLinecap="round"
          transform="rotate(-90 40 28)" />
        <circle cx="40" cy="28" r="14" fill="#e8f9f0" />
        <text x="40" y="33" textAnchor="middle" fontSize="11" fontWeight="bold" fill="#2da05b">Active</text>
      </svg>
    ),
  },
  leave: {
  ...
  new: {
    labelColor: "text-[#a07000]",
    metaIcon: "✨",
    metaColor: "text-[#555]",
    borderColor: "border-[#ffb800]",
    graphic: (
      <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 80 60" className="w-full h-full">
        <circle cx="40" cy="28" r="20" fill="#fff8e1" stroke="#ffb800" strokeWidth="2" />
        <text x="40" y="33" textAnchor="middle" fontSize="22" fontWeight="bold" fill="#ffb800">+</text>
      </svg>
    ),
  },
};

export function UserMetricCard({ label, value, meta, type }: UserMetricCardProps) {
  const cfg = configs[type];
  return (
    <article className={`relative overflow-hidden rounded-[1.25rem] bg-white px-5 pb-5 pt-5 border-b-[4px] ${cfg.borderColor} shadow-[0_8px_20px_-6px_rgba(0,0,0,0.1)]`}>
      <div className="absolute right-2 top-2 w-[72px] h-[54px] opacity-70 pointer-events-none">
        {cfg.graphic}
      </div>
      <p className={`text-[11.5px] font-extrabold uppercase tracking-widest ${cfg.labelColor}`}>{label}</p>
