...
// ─── Collapsible group ────────────────────────────────────────────────────────
function NavGroup({
  title, items, pathname, expandedGroup, setExpandedGroup, onNavClick,
}: {
  title: string;
...
export function Sidebar({ mobileOpen = false, onClose }: SidebarProps) {
  ...
  const adminGroups = useMemo(() => [
    ...
    { title: "Time & Attendance", items: ["Attendance", "Leave Requests"] },
    { title: "Operations", items: ["Inquiries", "Reports", "Announcements", "Audit Log"] },
  ].map((g) => ({
    ...g,
    items: g.items
  ...
  const teacherLinks = [
    ...
    { label: "Clock In/Out", href: "/teacher/clock", icon: <ClockIcon /> },
    { label: "History", href: "/teacher/history", icon: <HistoryIcon /> },
    { label: "Leaves", href: "/teacher/leaves", icon: <LeaveIcon /> },
        ...
        <div className="relative">
          ...
          >
            ...
            <div className="min-w-0 flex-1 text-left">
              <p className="truncate text-[13.5px] font-bold text-[#002f76] leading-tight">{displayName}</p>
